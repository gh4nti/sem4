...
CREATE
OR REPLACE PROCEDURE dept_details(p_dept_name IN VARCHAR2) IS -- Cursor for instructors
...
CREATE
OR REPLACE PROCEDURE course_popular(p_dept_name IN VARCHAR2) IS v_course_id course.course_id % TYPE;

v_title course.title % TYPE;

v_count NUMBER;

BEGIN -- Find most popular course in the given department
SELECT
	c.course_id,
	c.title,
...
CREATE
OR REPLACE PROCEDURE dept_student_course(p_dept_name IN VARCHAR2) IS -- Cursor for students
CURSOR c_student IS
SELECT
	name
FROM
...
BEGIN dept_student_course('Comp. Sci.');
...
CREATE
OR REPLACE FUNCTION department_highest(p_dept_name IN VARCHAR2) RETURN VARCHAR2 IS v_name instructor.name % TYPE;
...
EXCEPTION
WHEN NO_DATA_FOUND THEN RETURN 'No Instructor Found';
 ...
 */
CREATE
OR REPLACE PACKAGE dept_pkg IS -- (a) Procedure to list instructor names
...
/ CREATE
OR REPLACE PACKAGE BODY dept_pkg IS -- Procedure implementation
